...
#define WIRE_ADDR 0x21
#define JOYSTICK_BASE_ADDRESS 0x0A
#define JOYSTICK_COUNT 4
Joystick_ joy[JOYSTICK_COUNT] = {
	Joystick_(JOYSTICK_BASE_ADDRESS + 0x00, JOYSTICK_TYPE_JOYSTICK, 128, 0, false, false, false, false, false, false, false, false, false, false, false), // BASE MFD
...
uint8_t activeJoystick = Mode::Left;
void handleChangeMode(uint8_t mode)
{
	activeJoystick = mode;
}

void handleButtonStateChange(uint8_t mode, uint8_t button, uint8_t value)
{
	// Serial.print(mode);
	// Serial.print(button);
	...
	{
		joy[mode].pressButton(button);
	}
	else
	{
		joy[mode].releaseButton(button);
	}
}

void handleReceive(int cnt)
{
	if (cnt == 3)
	{
		...
		handleButtonStateChange(mode, button, value);
	}
	else
	{
		for (int i = 0; i < cnt; i++)
...
{
	handleButtonStateChange(Mode::Base, button, State::Pressed);
}
void handleBaseRelease(uint8_t button)
{
	handleButtonStateChange(Mode::Base, button, State::Released);
}
void handleBaseMomentaryPress(uint8_t button)
{
	handleBasePress(button);
	delay(16);
	handleBaseRelease(button);
	delay(1);
}
...
{
	handleButtonStateChange(activeJoystick, button, State::Pressed);
}
void handleDynamicRelease(uint8_t button)
{
	handleButtonStateChange(activeJoystick, button, State::Released);
}
